/*
 *  swta_neuron_dbl_exp.cpp
 *
 *  swta_neuron_dbl_exp: modified version of swta_neuron using double
 *  exponential PSP.
 */
...
RecordablesMap< swta_neuron_dbl_exp > swta_neuron_dbl_exp::recordablesMap_;

// Override the create() method with one call to RecordablesMap::insert_()
// for each quantity to be recorded.
template <>
void
RecordablesMap< swta_neuron_dbl_exp >::create()
{
  // use standard names whereever you can for consistency!
  insert_( names::V_m, &swta_neuron_dbl_exp::get_V_m_ );
  insert_( names::E_sfa, &swta_neuron_dbl_exp::get_E_sfa_ );
}
...
nest::swta_neuron_dbl_exp::Parameters_::Parameters_()
  : tau_r_( 1.0 )    // ms
  , tau_f_( 10.0 )    // ms
  , dead_time_( 1.0 ) // ms
  , dead_time_random_( 0 )
...
nest::swta_neuron_dbl_exp::State_::State_()
  : y0_(0.0)
  , y3_(0.0)
  , q_(0.0)
  , u_rise_ (0.0)
...
void
nest::swta_neuron_dbl_exp::Parameters_::get( DictionaryDatum& d ) const
{
  def< double >( d, names::I_e, I_e_ );
  def< double >( d, "tau_r", tau_r_ );
  def< double >( d, "tau_f", tau_f_ );
  ...
  def< double >( d, "c_4", c_4_ );
  def< double >( d, names::V_reset, V_reset_ );
  def< double >( d, "z_scale", z_scale_ );
  def< double >( d, "I_scale", I_scale_ );
  def< bool >( d, "E_sfa_clip", E_sfa_clip_ );
  def< double >( d, "E_sfa_max", E_sfa_max_ );
...
void
nest::swta_neuron_dbl_exp::Parameters_::set( const DictionaryDatum& d )
{

  updateValue< double >( d, names::I_e, I_e_ );
  updateValue< double >( d, "tau_r", tau_r_ );
  updateValue< double >( d, "tau_f", tau_f_ );
  ...
  updateValue< double >( d, "c_4", c_4_ );
  updateValue< double >( d, names::V_reset, V_reset_ );
  updateValue< double >( d, "z_scale", z_scale_ );
  updateValue< double >( d, "I_scale", I_scale_ );
  updateValue< bool >( d, "E_sfa_clip", E_sfa_clip_ );
  updateValue< double >( d, "E_sfa_max", E_sfa_max_ );
...
void
nest::swta_neuron_dbl_exp::State_::get( DictionaryDatum& d, const Parameters_& ) const
{
...
void
nest::swta_neuron_dbl_exp::State_::set( const DictionaryDatum& d, const Parameters_& )
{
...
nest::swta_neuron_dbl_exp::Buffers_::Buffers_( swta_neuron_dbl_exp& n )
  : logger_( n )
{
}

nest::swta_neuron_dbl_exp::Buffers_::Buffers_( const Buffers_&, swta_neuron_dbl_exp& n )
  : logger_( n )
{
}
...
nest::swta_neuron_dbl_exp::swta_neuron_dbl_exp()
  : Archiving_Node()
  , P_()
  , S_()
  , B_( *this )
...
nest::swta_neuron_dbl_exp::swta_neuron_dbl_exp( const swta_neuron_dbl_exp& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::swta_neuron_dbl_exp::init_state_( const Node& proto )
{
  const swta_neuron_dbl_exp& pr = downcast< swta_neuron_dbl_exp >( proto );
  S_ = pr.S_;
  S_.r_ = Time( Time::ms( P_.t_ref_remaining_ ) ).get_steps();
}

void
nest::swta_neuron_dbl_exp::init_buffers_()
{
...
void
nest::swta_neuron_dbl_exp::calibrate()
{
  ...
  // quantals used for updating rise and fall states
  V_.u_rise_dt_ = std::exp(-V_.h_ / P_.tau_r_);
  V_.u_fall_dt_ = std::exp(-V_.h_ / P_.tau_f_);
  ...
  if ( not S_.initialized_ )
  {
    if (V_.Q33_.size() > 0)
    {
      V_.Q33_.clear(); // note: we lose information here!
    }
    if (S_.q_elems_.size() > 0)
    {
      S_.q_elems_.clear(); // note: we lose information here!
    }
...
void
nest::swta_neuron_dbl_exp::update( Time const& origin, const long from, const long to )
{
  ...
  {
    // get spikes
    double psc = B_.spikes_.get_value(lag);

    // advance states of rise and fall contributions
    S_.u_rise_ = (S_.u_rise_ * V_.u_rise_dt_) + psc;
    S_.u_fall_ = (S_.u_fall_ * V_.u_fall_dt_) + psc;

    // membrane potential update:
    S_.y3_ = P_.z_scale_ * (S_.u_fall_ - S_.u_rise_)
           + P_.I_scale_ * (S_.y0_ + P_.I_e_);
    ...
    // clip E_sfa according to settings
    if (P_.E_sfa_clip_)
    {
        // clipping depends on sign of E_sfa_max_:
        //   E_sfa_max_ < 0: lower bound
        //   E_sfa_max_ > 0: upper bound
        if (P_.E_sfa_max_ < 0. && q_temp_ < P_.E_sfa_max_)
        {
            q_temp_ = P_.E_sfa_max_;
        }
        else if (P_.E_sfa_max_ > 0. && q_temp_ > P_.E_sfa_max_)
        {
            q_temp_ = P_.E_sfa_max_;
        }
    }
    ...
    if ( S_.r_ == 0 )
    {
      ...
      // rate of pp_psc_delta:
      //double rate = ( P_.c_1_ * V_eff + P_.c_2_ * std::exp( P_.c_3_ * V_eff ) );

      // modified rate:
      double rate = P_.c_1_ * V_eff + P_.c_2_ * (std::exp(P_.c_3_ * V_eff) - P_.c_4_);
      ...
      if ( rate > 0.0 )
      {
        ...
        if ( P_.dead_time_ > 0.0 )
        {
          ...
          if ( V_.rng_->drand() <= -numerics::expm1( -rate * V_.h_ * 1e-3 ) )  // continuous time
          //if ( V_.rng_->drand() < rate * V_.h_ * 1e-3 )                      // simplified continuous time
          //if ( V_.rng_->drand() < 1. / (1 + exp(- rate) * V_.h_ * 1e-3) )    // discrete time
          {
            n_spikes = 1;
          }
        }
...
void
nest::swta_neuron_dbl_exp::handle( SpikeEvent& e )
{
...
void
nest::swta_neuron_dbl_exp::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
nest::swta_neuron_dbl_exp::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
